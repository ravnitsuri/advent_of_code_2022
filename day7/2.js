...
let main = async () => {
  try {
    ...
    let minRemainingSize = 30000000;
    let totalTreeSize = getSize(tree, (name, size) => {
      sizes.push({ name, size });
    });

    let remainingSize = maxSize - totalTreeSize;
    if (remainingSize > minRemainingSize) throw new Error("its already freeee");

    let mustDeleteSize = minRemainingSize - remainingSize

    console.log({ remainingSize, mustDeleteSize });
    let output = sizes
      .sort((a, b) => a.size - b.size)
      .filter((x) => x.size > mustDeleteSize)[0].size
